let replace_with_rule = (rule, input) => {
  ...
  let rec loop = (i, output) => {
    ...
    | _ => {
        ...
        let from = r["from"]
        let to_ = r["to"]
        let rgx = Js.Re.fromStringWithFlags(from, ~flags="g")
        loop(i + 1, output->Js.String2.replaceByRe(rgx, to_))
      }
    }
  }
  loop(0, input)
...
let uni2zg: string => string = input => {
  let rule = [
    {
      "from": `\u1004\u103a\u1039`,
      "to": `\u1064`,
  ...
  replace_with_rule(rule, input)
}

let zg2uni: string => string = input => {
  let rule = [
    {
      "from": `([\u102D\u102E\u103D\u102F\u1037\u1095])\\\1+`,
      "to": `$1`,
